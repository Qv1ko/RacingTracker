...
import { Driver, Participation, Race, Team } from "@/types";
...
export default function CreateRaceForm({
    ...
    const { data, setData, processing, put, errors } = useForm({
        ...
        result: participations.map((participation) => ({
            position: participation.status.toString(),
            driver: participation.driver_id.toString(),
            team: participation.team_id?.toString() || "",
        })),
    });
    ...
    const sortResult = (
        a: { position: string; driver: string; team: string },
        b: { position: string; driver: string; team: string },
    ) => {
    ...
    const isNumericPosition = (position: string) =>
        typeof position === "number" || (typeof position === "string" && !isNaN(Number(position)));
                            ...
                            <Button
                                ...
                                onClick={() =>
                                    setData(
                                        "result",
                                        correctPositions(
                                            data.result.concat(newParticipant).sort(sortResult),
                                        ),
                                    )
                                }
                            >
                    ...
                    <table className="w-full">
                        <thead>
                            <tr>
                                ...
                                <th className="hidden text-sm leading-none font-medium sm:table-cell">
                                    Driver
                                </th>
                                <th className="hidden text-sm leading-none font-medium sm:table-cell">
                                    Team
                                </th>
                                <th className="table-cell text-sm leading-none font-medium sm:hidden">
                                    Participant
                                </th>
                                <th className="w-0"></th>
                            </tr>
                        ...
                        <tbody>
                            {data.result.map((participation, i) => (
                                <tr
                                    key={"participation-" + i}
                                    className="hover:bg-muted/50 mt-2 transition-colors"
                                >
